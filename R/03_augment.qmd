...
The data set is to big for the shiny app, so we take a random sample to use:

```{r}
clean_data_shiny <- clean_data |> 
  sample_n(10000)
write_csv(clean_data_shiny, here("data/03_dat_aug_shiny.csv"))
```
